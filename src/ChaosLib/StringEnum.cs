...
namespace ChaosLib
{
    ...
    public struct StringEnum<TEnum> : IEquatable<StringEnum<TEnum>>, IStringEnum where TEnum : struct
    {
        private bool split;
        ...
        /// <param name="defaultValue">A lambda that returns the default value to use if <paramref name="stringValue"/> does not correspond to a known enum value.</param>
        public StringEnum(string stringValue, Func<TEnum> defaultValue = null)
        {
            if (stringValue == null)
            ...
            var val = Deserialize(stringValue, out var strVal);
            StringValue = strVal;
        ...
        {
            if (((Enum) (object) enumValue).TryGetDescription(out var description))
                return description;

            return enumValue.ToString().ToLower();
        }

        private static object Deserialize(string str, out string stringValue)
        {
            stringValue = null;
            ...
            {
                if (((Enum) (object) enumValue).TryGetDescription(out var description) && description == str)
                    return enumValue;
            }
            ...
            {
                if (((Enum) (object) value).TryGetDescription(out var description))
